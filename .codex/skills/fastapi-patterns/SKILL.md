...
> **Note**: The following project structure is **for reference only**.  
> In real-world development you are free to adopt or adapt it; the key point is to keep a **standardized and modular layout**.  
> Personally I prefer `backend/` over `app/` as the top-level package name, but the sub-folder names such as `models/`, `schemas/`, `api/`, `core/` etc. are worth keeping because they are intuitive and widely recognized.
...
class AppException(Exception):
    def __init__(self, status_code: int, detail: str, issue_code: str | None = None):
